...
class itempropwp
{
    ...
    public static function init()
    {
         ...
         */
        if (is_admin()):
            include_once($itempropwpn->directoriesloc('ipwpm') . '/admin/adminipwp.php');
        endif;
        /*
         * itempropwp CSS
         * @since 3.2.0
         ...
         */
        if (!is_admin()):
            wp_register_style('itempropwp', $itempropwpn->directoriesloc('ipwpurl') . 'assets/css/itempropwp.css', array(), self::VERSION, 'all');
            wp_enqueue_style('itempropwp');
        endif;

        load_plugin_textdomain('itempropwp', false, $itempropwpn->directoriesloc('ipwpdir') . '/lang/');
        add_filter('the_content', array($itempropwpn, 'ipwp_the_content_filter'), 10, 2); // Adding context @since 3.0
    }
